...
local waitTable = {};
local waitFrame = nil;
local waitForEntries = true;

-- LOAD FUNCTION --
function WanderersGambling_OnLoad(self)
	DEFAULT_CHAT_FRAME:AddMessage("|cffffff00<Wanderers Gambling 0.65> /gamble to use");
		...
		if(AcceptOnes=="true") then
			...
			if (arg1 == "1") then
				if(WanderersGambling_ChkBan(tostring(arg2)) == 0) then
					...
					lastentry = time();
					-- SendChatMessage(strjoin(" ", "Got a role at", lastentry), chatmethod); --
...
function WanderersGambling_CheckForRolls()
	local nowTime = time();
	local endTime = lastentry + 10;
	if ( waitForEntries ) then
		if ( totalrolls >= 2) then
			-- SendChatMessage(endTime - nowTime, chatmethod); --
			if ( nowTime >= endTime ) then
				-- We have waited 10 seconds since our last entry
				waitForEntries = false;
				WanderersGambling_OnClickLASTCALL();
				C_Timer.After(5, WanderersGambling_OnClickROLL);
			end
		end
		C_Timer.After(1 , WanderersGambling_CheckForRolls);
	end
end

function WanderersGambling_OnClickAutomate()
	WanderersGambling_OnClickACCEPTONES();
	waitForEntries = true;
	SendChatMessage("Waiting 10 seconds from last entry...", chatmethod);
	C_Timer.After(1, WanderersGambling_CheckForRolls);
end
...
function WanderersGambling_OnClickACCEPTONES()
	if WanderersGambling_EditBox:GetText() ~= "" and WanderersGambling_EditBox:GetText() ~= "1" then
		...
		AcceptLoserAmount = "false";
		local fakeroll = "";
		SendChatMessage(format("%s%s%s%s", ".:Welcome to WanderersGambling:. User's Roll - (", BreakUpLargeNumbers(tonumber(WanderersGambling_EditBox:GetText())), ") - Type 1 to Join  (-1 to withdraw)", fakeroll),chatmethod,GetDefaultLanguage("player"));
...
function WanderersGambling_Report()
	...
	if (goldowed ~= 0) then
		...
		SetRaidTarget(lowname, 8);
		SetRaidTarget(highname, 2);
...
function WanderersGambling_ResetCmd()
	SendChatMessage(".:WanderersGambling:. Game has been reset", chatmethod)
end
